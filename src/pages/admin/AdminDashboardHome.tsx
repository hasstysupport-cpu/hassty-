...
export const AdminDashboardHome: React.FC<AdminDashboardHomeProps> = ({
  ...
  const totalUsers = studentsCount + parentsCount + teachersCount;

  // Compute dynamic chart data from real accounts
  const chartData = useMemo(() => {
    if (totalUsers === 0) {
      return [
        { period: 'بداية المنصة', students: 0, parents: 0, teachers: 0 },
        { period: 'الحالي', students: 0, parents: 0, teachers: 0 }
      ];
    }
    return [
      { period: 'السابق', students: 0, parents: 0, teachers: 0 },
      { period: 'الحالي', students: studentsCount, parents: parentsCount, teachers: teachersCount },
    ];
  ...
  return (
    <div className="space-y-6 text-right font-['IBM_Plex_Sans_Arabic',sans-serif]">
      ...
      <div className="bg-[#1E3A8A] text-white rounded-3xl p-6 sm:p-7 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-xs text-blue-200">بيانات حية مباشرة من قاعدة البيانات</span>
          </div>
          <h2 className="text-xl sm:text-2xl font-black text-white">
            نظرة عامة على نشاط المنصة 📊
          </h2>
          <p className="text-xs text-blue-200 mt-1 max-w-xl">
            متابعة حركة الحسابات المسجلة فعلياً، تدقيق هويات المدرسين، وإدارة أمان المنصة.
          </p>
        </div>
      ...
      {/* 2. Top Stats Grid (Real Dynamic Numbers) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
        
        {/* Total Registered Accounts */}
        <div className="bg-white border border-gray-200 rounded-3xl p-5 shadow-xs space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-gray-500">إجمالي الحسابات المسجلة</span>
            <div className="w-8 h-8 rounded-xl bg-blue-50 text-[#2563EB] flex items-center justify-center">
              <Users className="w-4 h-4" />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-2xl sm:text-3xl font-black text-[#1E3A8A]">{totalUsers}</span>
            <span className="text-[11px] font-bold text-emerald-600 flex items-center gap-0.5">
              حساب نشط
            </span>
          </div>
          <p className="text-[10px] text-gray-400">إجمالي المستخدمين في قاعدة البيانات</p>
        </div>

        {/* Signups Breakdown */}
        <div className="bg-white border border-gray-200 rounded-3xl p-5 shadow-xs space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-gray-500">توزيع الحسابات</span>
        ...
        <div className="bg-white border border-gray-200 rounded-3xl p-5 shadow-xs space-y-2">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-[11px] font-bold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full">
              {pendingVerifications.length === 0 ? 'لا توجد طلبات' : 'بانتظار المراجعة'}
            </span>
          </div>
          <p className="text-[10px] text-gray-400">طلبات توثيق الهوية الوطنية</p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-3xl p-5 shadow-xs space-y-2">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-[11px] font-bold text-red-700 bg-red-50 px-2 py-0.5 rounded-full">
              {pendingReports.length === 0 ? 'مستقر وآمن' : 'تتطلب تدخلاً'}
            </span>
          </div>
          <p className="text-[10px] text-gray-400">شكاوى سوء سلوك أو بلاغات أمان</p>
        </div>
      </div>

      {/* 3. Main Chart: Growth Over Time (Real dynamic data) */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            <h3 className="text-base font-black text-[#1E3A8A]">
              توزيع الحسابات الحالية حسب الفئة (User Distribution)
            </h3>
            <p className="text-xs text-gray-500">مخطط بياني دقيق يمثل الحسابات المسجلة في المنصة</p>
          </div>
          <div className="flex items-center gap-4 text-xs font-bold">
            <span className="flex items-center gap-1.5 text-blue-600">
              <span className="w-3 h-3 rounded-full bg-blue-600" /> طلاب ({studentsCount})
            </span>
            <span className="flex items-center gap-1.5 text-amber-500">
              <span className="w-3 h-3 rounded-full bg-amber-500" /> أولياء أمور ({parentsCount})
            </span>
            <span className="flex items-center gap-1.5 text-emerald-500">
              <span className="w-3 h-3 rounded-full bg-emerald-500" /> مدرسين ({teachersCount})
            </span>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-3xl p-5 shadow-xs space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h4 className="text-sm font-black text-[#1E3A8A]">طلبات توثيق المدرسين</h4>
                <p className="text-[11px] text-gray-500">طلبات التوثيق ببطاقة الرقم القومي</p>
              </div>
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-3xl p-5 shadow-xs space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h4 className="text-sm font-black text-[#1E3A8A]">البلاغات والشكاوى</h4>
          ...
          <div className="space-y-2.5">
            {pendingReports.length === 0 ? (
              <div className="p-8 text-center text-xs text-gray-400 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                لا توجد بلاغات مسجلة حالياً، المنصة آمنة ومستقرة ✨
              </div>
            ) : (
