...
export const TeacherEnrollmentRequestsPage: React.FC<{ onNavigate?: (p: string) => void }> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`enrollment-req-${user.uid}-${Date.now().toString(36)}`).on('postgres_changes', { event: '*', schema: 'public', table: 'booking_requests', filter: `tutor_id=eq.${user.uid}` }, () => void load()).subscribe();
