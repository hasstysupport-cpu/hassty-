...
import { MessageCircle, Search, Send, UserCheck, Users, MapPin, BriefcaseBusiness } from 'lucide-react';
...
export const TeacherAssistantsPage: React.FC<{ onNavigate: (path: string) => void }> = ({ onNavigate }) => {
  ...
  const load = async () => {
    ...
    const [{ data: a }, { data: l }] = await Promise.all([
      ...
      supabase.from('teacher_assistants').select('*').eq('teacher_id', user.uid).eq('status', 'active'),
    ]);
  ...
  const filtered = useMemo(() => assistants.filter(a => {
    ...
    return (!search || hay.includes(search.trim().toLowerCase())) && (!location || `${a.governorate || ''} ${a.city || ''}`.includes(location));
  ...
  return <div className="space-y-6 text-right" dir="rtl">
    <section className="bg-white rounded-3xl border border-slate-200 p-6"><div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4"><div><div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-50 text-blue-700 text-xs font-black"><Users className="w-4 h-4"/>فريق المساعدين</div><h1 className="text-2xl font-black text-[#1E3A8A] mt-2">البحث عن مساعد مدرس</h1><p className="text-sm text-slate-500 mt-1">ابحث عن مساعدين موثقين وتواصل معهم قبل إرسال دعوة رسمية.</p></div><button onClick={()=>onNavigate('/teacher/dashboard')} className="px-4 py-2.5 rounded-xl border border-slate-200 text-xs font-black">العودة للوحة المدرس</button></div></section>
    ...
    <section className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">{filtered.map(a=>{const linked=links.some(l=>l.assistant_id===a.user_id);return <article key={a.user_id} className="bg-white rounded-3xl border border-slate-200 p-5 shadow-sm"><div className="flex items-start gap-3"><div className="w-12 h-12 rounded-2xl bg-blue-50 text-blue-700 flex items-center justify-center"><BriefcaseBusiness className="w-6 h-6"/></div><div className="min-w-0 flex-1"><h2 className="font-black text-slate-900 truncate">{a.full_name}</h2><div className="text-xs text-emerald-700 font-black mt-1 flex items-center gap-1"><UserCheck className="w-3.5 h-3.5"/>مساعد موثق</div></div></div><div className="mt-4 space-y-2 text-xs text-slate-600"><div className="flex gap-2"><MapPin className="w-4 h-4 text-slate-400"/>{a.governorate || '—'} {a.city ? `— ${a.city}` : ''}</div><div><b>المؤهل:</b> {a.education || 'غير مذكور'}</div><div><b>الخبرة:</b> {a.experience_years || 0} سنة</div>{a.experience_summary&&<p className="leading-6 text-slate-500">{a.experience_summary}</p>}</div><div className="grid grid-cols-2 gap-2 mt-5"><button disabled={!!busy || linked} onClick={()=>void invite(a.user_id)} className="rounded-xl bg-blue-600 text-white py-2.5 text-xs font-black disabled:opacity-50">{linked?'مرتبط بالفعل':busy===a.user_id?'جارٍ...':'إرسال دعوة'}</button><button disabled={!!busy} onClick={()=>void startChat(a.user_id)} className="rounded-xl border border-emerald-200 bg-emerald-50 text-emerald-800 py-2.5 text-xs font-black flex items-center justify-center gap-1"><MessageCircle className="w-4 h-4"/>مراسلة</button></div></article>})}</section>
