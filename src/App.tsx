...
export default function App() {
  ...
  const handleLogin = (role: AccountRole) => { if(role==='admin') setCurrentPath(SECRET_ADMIN_ROUTE); else if(role==='teacher') setCurrentPath('/teacher/dashboard'); else if(role==='parent') setCurrentPath('/parent/dashboard'); else if(role==='assistant') setCurrentPath('/assistant/dashboard'); else setCurrentPath('/student/dashboard'); };
  const handleLogout = () => { void logout(); setNeedsProfileSetup(false); setCurrentPath('/'); };
  const handleSelectTutor = (tutorId:string) => { setSelectedTutorId(tutorId); setCurrentPath(`/tutor/${tutorId}`); };
  ...
  const isAssistantSignupRoute = currentPath === '/assistant/signup';

  useEffect(() => {
    let cancelled=false;
    const check=async()=>{
      if(!user?.uid || user.role==='admin' || user.role==='assistant' || !supabase){ if(!cancelled){setNeedsProfileSetup(false);setIsCheckingProfile(false);} return; }
      ...
      try {
        ...
        const metadata=(data?.metadata||{}) as Record<string,any>; const role=(data?.role||user.role) as AccountRole;
        const commonComplete=Boolean(data?.full_name?.trim()&&data?.phone?.trim()&&data?.governorate?.trim()&&data?.city?.trim());
        const roleComplete=role==='teacher' ? Boolean((metadata.subject||user.profileData?.subject)?.toString().trim()&&(metadata.experienceYears||user.profileData?.experienceYears)?.toString().trim()) : role==='student' ? Boolean((data?.grade||metadata.grade||user.profileData?.grade)?.toString().trim()) : true;
        if(!cancelled) setNeedsProfileSetup(!data||!commonComplete||!roleComplete);
      } catch(err){ console.warn('Profile completion check failed:',err); if(!cancelled) setNeedsProfileSetup(false); }
      finally{ if(!cancelled) setIsCheckingProfile(false); }
    }; void check(); return()=>{cancelled=true};
  },[user]);
  useEffect(()=>{ if(!user?.uid||!signupLegalAccepted||user.role==='admin') return; void recordRequiredSignupConsents(user.uid).catch((error)=>console.warn('Legal consent audit warning:',error)); },[user?.uid,signupLegalAccepted,user?.role]);
  ...
  useEffect(()=>{ if(isLoggedIn&&!isUnverified&&!needsProfileSetup&&!isCheckingProfile){ if(currentPath==='/login'||currentPath==='/signup'||currentPath==='/setup-profile') handleLogin(currentRole); } },[isLoggedIn,currentRole,currentPath,isUnverified,needsProfileSetup,isCheckingProfile]);
  ...
  return <div className="min-h-screen bg-[#F8FAFF] text-[#1F2937] selection:bg-[#EFF6FF] selection:text-[#2563EB] flex flex-col antialiased">
    {isLoggedIn&&isDashboardRoute&&!isUnverified&&!needsProfileSetup?<LoggedInNavbar currentRole={currentRole} currentPath={currentPath} userName={user?.name} userAvatar={user?.avatarUrl||user?.profileData?.avatarUrl} onNavigate={handleNavigate} onRoleChange={(newRole)=>setCurrentPath(`/${newRole}/dashboard`)} onLogout={handleLogout}/>:<PublicNavbar currentPath={currentPath} isLoggedIn={isLoggedIn&&!isUnverified&&!needsProfileSetup} user={user} currentRole={currentRole} onNavigate={handleNavigate} onOpenLogin={()=>handleNavigate('/login')} onOpenSignup={()=>handleNavigate('/signup')} onLogout={handleLogout}/>} 
    {isDashboardRoute&&isLoggedIn&&!isUnverified&&!needsProfileSetup?<div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 pb-24 sm:pb-28 lg:pb-8 flex flex-col md:flex-row gap-6 lg:gap-8 items-start"><DashboardSidebar currentRole={currentRole} currentPath={currentPath} onNavigate={handleNavigate} onLogout={handleLogout}/><main className="flex-1 w-full min-w-0 pb-6 page-transition">
      ...
      {currentPath==='/teacher/dashboard'&&<TeacherDashboardPage onNavigate={handleNavigate}/>} {currentPath==='/teacher/students'&&<TeacherStudentsPage onNavigate={handleNavigate}/>} {currentPath==='/teacher/groups'&&<TeacherGroupsPage/>} {currentPath==='/teacher/scan'&&<TeacherScanPage/>} {currentPath==='/teacher/attendance'&&<TeacherAttendancePage/>} {currentPath==='/teacher/payments'&&<TeacherPaymentsPage onNavigate={handleNavigate}/>} {currentPath==='/teacher/availability'&&<TeacherAvailabilityPage/>} {currentPath==='/teacher/profile'&&<TeacherProfileEditPage/>} {currentPath==='/teacher/reviews'&&<TeacherReviewsPage/>} {currentPath==='/teacher/notifications'&&<NotificationsPage onNavigate={handleNavigate}/>} {currentPath==='/teacher/calendar'&&<CalendarPage/>} {currentPath==='/teacher/messages'&&<MessagesPage/>} {currentPath==='/teacher/assignments'&&<TeacherAssignmentsPage/>}
      {currentPath==='/assistant/dashboard'&&<div className="rounded-3xl bg-white border border-slate-200 p-8"><h1 className="text-2xl font-black text-[#1E3A8A]">لوحة المساعد</h1><p className="mt-2 text-sm text-slate-500">سيتم تفعيل لوحة المساعد بعد اعتماد الحساب وربطه بمدرس.</p></div>}
