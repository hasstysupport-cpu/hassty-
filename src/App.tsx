...
export default function App(){
 ...
 return <ToastProvider><div data-role={currentRole} className="min-h-screen bg-[#F8FAFF] text-[#1F2937] flex flex-col antialiased">
  ...
  {/* بانر إذن إشعارات المتصفح: في كل صفحات الموقع للمسجلين (مش الداشبورد فقط) */}
  ...
  </Suspense></main></div>:<main key={currentPath} className="flex-1 page-transition"><Suspense fallback={<PageLoader/>}>{isCheckingProfile&&isLoggedIn&&!isUnverified&&isKnownPublicPath&&currentPath!=='/setup-profile'&&<div className="max-w-3xl mx-auto px-4 py-8 text-center text-xs text-slate-500">جاري تجهيز بيانات حسابك...</div>} {currentPath==='/'&&<HomePage onNavigate={handleNavigate} onSelectTutor={handleSelectTutor} onSearchWithParams={handleSearchWithParams}/>} {currentPath==='/search'&&<SearchResultsPage onNavigate={handleNavigate} onSelectTutor={handleSelectTutor} initialSubject={searchSubject} initialGovernorate={searchGovernorate} initialCity={searchCity}/>} {currentPath.startsWith('/tutor')&&<TeacherProfilePage tutorId={selectedTutorId} onNavigate={handleNavigate} onSelectTutor={handleSelectTutor}/>} {currentPath==='/about'&&<AboutPage onNavigate={handleNavigate}/>} {currentPath==='/contact'&&<ContactPage onNavigate={handleNavigate}/>} {currentPath==='/for-teachers'&&<ForTeachersPage onNavigate={handleNavigate}/>} {currentPath==='/login'&&<LoginPage onNavigate={handleNavigate} onLoginSuccess={handleLogin}/>} {(currentPath==='/verify-email'||isUnverified)&&<VerifyEmailPage onNavigate={handleNavigate} onVerificationSuccess={handleLogin}/>} {currentPath==='/whatsapp-studio'&&<Suspense fallback={<PageLoader/>}><WhatsAppStudioPage/></Suspense>} {currentPath==='/qr-attendance'&&<QrAttendancePage onNavigate={handleNavigate}/>}{!isKnownPublicPath&&!isUnverified&&<NotFoundPage onNavigate={handleNavigate}/>}</Suspense></main>}
