...
export default function App() {
  ...
  const handleLogin = (role: AccountRole) => { if(role==='admin') setCurrentPath(SECRET_ADMIN_ROUTE); else if(role==='teacher') setCurrentPath('/teacher/dashboard'); else if(role==='parent') setCurrentPath('/parent/dashboard'); else setCurrentPath('/student/dashboard'); };
  ...
  useEffect(() => {
    let cancelled=false;
    const check=async()=>{
      if(!user?.uid || user.role==='admin' || !supabase){ if(!cancelled){setNeedsProfileSetup(false);setIsCheckingProfile(false);} return; }
  ...
  return <div className="min-h-screen bg-[#F8FAFF] text-[#1F2937] font-['IBM_Plex_Sans_Arabic',sans-serif] selection:bg-[#EFF6FF] selection:text-[#2563EB] flex flex-col antialiased">
