...
function legalDocument(kind: LegalKind) {
  ...
  if (kind === 'terms') return {
    ...
    sections: [
      ...
      ['12. التواصل', 'للاستفسارات والشكاوى المتعلقة بالشروط: جروب الدعم الرسمي على تليجرام'],
    ],
  };
  ...
  if (kind === 'privacy') return {
    ...
    sections: [
      ...
      ['11. التواصل', 'لطلبات الخصوصية وحماية البيانات: جروب الدعم الرسمي على تليجرام. سنعمل على توثيق الطلب ومتابعته وفق الإجراءات والمهل القانونية المنطبقة.'],
  ...
  const generic: Record<string, { title: string; icon: React.ReactNode; intro: string; sections: string[][] }> = {
    ...
    refund: { title: 'الدفع والاسترداد', icon: <Scale className="w-7 h-7" />, intro: 'نريد أن تكون رسوم الخدمة وحالات الإلغاء والاسترداد واضحة قبل تأكيد أي عملية.', sections: [
      ...
      ['نزاعات الدفع', 'إذا حدثت مشكلة في عملية دفع، احتفظ بإثبات العملية وتواصل معنا فورًا عبر جروب الدعم الرسمي على تليجرام.'],
    ] },
    ...
    rights: { title: 'حقوق البيانات وطلبات المحو', icon: <Eye className="w-7 h-7" />, intro: 'نحن نريد أن يكون للمستخدم طريق واضح لمعرفة بياناته والتحكم فيها ضمن الحدود القانونية.', sections: [
      ...
      ['كيف تقدم الطلب؟', 'أرسل طلبك عبر جروب الدعم الرسمي على تليجرام مع وصف الطلب وبيانات الحساب. قد نطلب تحققًا مناسبًا من الهوية قبل الإفصاح أو التعديل أو الحذف.'],
    ] },
  };
      ...
      {path === '/signup' && (
        <div className="fixed bottom-3 left-3 right-3 sm:left-auto sm:right-5 sm:w-[430px] z-[110]" dir="rtl">
          <div className="rounded-3xl bg-white border border-slate-200 shadow-2xl p-4 sm:p-5">
            ...
            <div className="mt-3 text-[10px] text-slate-500 flex items-center gap-1"><Send className="w-3 h-3" /> طلبات الخصوصية: جروب الدعم الرسمي على تليجرام <Trash2 className="w-3 h-3 mr-2" /> طلبات المحو متاحة وفق القانون</div>
          </div>
        </div>
      )}
    </>
