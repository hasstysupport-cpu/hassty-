...
import { Menu, X, ArrowLeft, User, Search, Sparkles, LogIn, UserPlus, LayoutDashboard, LogOut } from 'lucide-react';
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center gap-3">
            {isLoggedIn ? (
              <div className="flex items-center gap-3">
                <button
                  onClick={() => handleLinkClick(getDashboardPath())}
                  className="px-4 py-2.5 text-xs font-bold text-white bg-[#2563EB] hover:bg-[#1D4ED8] rounded-xl transition-all shadow-sm flex items-center gap-2 cursor-pointer"
                >
                  <LayoutDashboard className="w-4 h-4" />
                  <span>الذهاب إلى {getRoleTitle()}</span>
                </button>

                <div 
                  onClick={() => handleLinkClick(getDashboardPath())}
                  className="flex items-center gap-2 p-1.5 pr-2.5 bg-gray-50 border border-gray-200 rounded-xl cursor-pointer hover:border-blue-300 transition-colors"
                >
                  <img
                    src={userAvatar}
                    alt={userName}
                ...
                {onLogout && (
                  <button
                    onClick={onLogout}
                    title="تسجيل الخروج"
                    className="p-2.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors cursor-pointer border border-transparent hover:border-red-200"
                  >
          ...
          <div className="flex md:hidden items-center gap-2">
            {isLoggedIn && (
              <button
                onClick={() => handleLinkClick(getDashboardPath())}
                className="p-1.5 rounded-xl border border-blue-200 bg-blue-50 text-blue-700 flex items-center gap-1 text-xs font-bold"
              >
                <img
                  src={userAvatar}
                  alt={userName}
                  ...
                  referrerPolicy="no-referrer"
                />
                <span className="hidden xs:inline truncate max-w-[80px]">{userName}</span>
              </button>
            )}
            <button
            ...
            <div className="p-3 bg-blue-50/70 border border-blue-100 rounded-2xl flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] font-semibold text-blue-600">{getRoleTitle()}</span>
                </div>
              </div>
              <button
                onClick={() => handleLinkClick(getDashboardPath())}
          ...
          <div className="pt-3 border-t border-gray-100">
            {isLoggedIn ? (
              <div className="space-y-2">
                <button
                  onClick={() => handleLinkClick(getDashboardPath())}
                  className="w-full py-3 text-center text-xs font-black text-white bg-[#2563EB] hover:bg-[#1D4ED8] rounded-xl cursor-pointer flex items-center justify-center gap-2"
                >
                  <LayoutDashboard className="w-4 h-4" />
                ...
                {onLogout && (
                  <button
                    ...
                    className="w-full py-2.5 text-center text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 rounded-xl cursor-pointer flex items-center justify-center gap-1.5"
                  >
