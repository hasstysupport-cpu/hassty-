...
export async function signInWithRedirect(_auth: any, provider: any) {
  ...
  const isLoginFlow = typeof window !== 'undefined' && window.location.pathname === '/login';
  if (isLoginFlow && typeof window !== 'undefined') {
    localStorage.setItem('hassty_google_login_started_at', String(Date.now()));
  }
  const redirectUrl = new URL(`${window.location.origin}/`);
  if (isLoginFlow) redirectUrl.searchParams.set('googleLogin', '1');
  const { error } = await supabase.auth.signInWithOAuth({ provider: providerName, options: { redirectTo: redirectUrl.toString() } });
  if (error) throw error;
  return null as any;
}
