...
export const dbService = {
  ...
  async getAttendanceRecords(qrCode?:string, studentId?:string): Promise<AttendanceRecord[]> {
    ...
    if(tp.error)throw tp.error;if(gp.error)throw gp.error; const tutorMap=new Map((tp.data||[]).map((p:any)=>[p.id,p.full_name||'المدرس'])); const groupMap=new Map((gp.data||[]).map((g:any)=>[g.id,g]));
    return rows.map((r:any)=>{const g=groupMap.get(r.group_id)||{};return {id:r.id,studentId:r.student_id||'',studentName:r.student_name||'طالب',tutorId:r.tutor_id||'',tutorName:tutorMap.get(r.tutor_id)||'المدرس',groupName:g.name||'المجموعة',date:r.date||'',time:r.time||'',subject:g.subject||'الحصة الدراسية',status:r.status,timeWindowStatus:r.status==='present'?'on_time':r.status==='late'?'late':'absent_cutoff',location:g.location||'',teacherNotes:r.notes||'',homeworkAssigned:r.homework_status==='completed'?'تم تسليم الواجب':r.homework_status==='partial'?'تسليم جزئي':'لم يتم التسليم'};});
  },
