...
create policy avd_assistant_select on public.assistant_verification_documents for select to authenticated using (assistant_id = (select auth.uid()));
create policy avd_assistant_insert on public.assistant_verification_documents for insert to authenticated with check (
...
create policy avd_admin_select on public.assistant_verification_documents for select to authenticated using (private.is_admin_strict());
create policy avd_admin_update on public.assistant_verification_documents for update to authenticated using (private.is_admin_strict()) with check (private.is_admin_strict());
...
create policy assistant_verification_upload on storage.objects for insert to authenticated with check (
  bucket_id='assistant-verification' and (storage.foldername(name))[1]=(select auth.uid()::text)
  and exists (select 1 from public.profiles p where p.id=(select auth.uid()) and p.role='assistant')
);
create policy assistant_verification_read on storage.objects for select to authenticated using (
  bucket_id='assistant-verification' and (owner_id=(select auth.uid()::text) or private.is_admin_strict())
);
create policy assistant_verification_delete on storage.objects for delete to authenticated using (
  bucket_id='assistant-verification' and owner_id=(select auth.uid()::text)
);

alter table public.assistant_verification_requests add column if not exists documents_submitted_at timestamptz;
alter table public.assistant_verification_requests add column if not exists id_card_review_status text;
alter table public.assistant_verification_requests add column if not exists qualification_review_status text;
alter table public.assistant_verification_requests add column if not exists last_admin_note_at timestamptz;
...
create trigger trg_assistant_verification_document_updated_at before update on public.assistant_verification_documents
...
begin
  if new.status='approved' then
    if coalesce(new.id_card_review_status,'')<>'accepted' or coalesce(new.qualification_review_status,'')<>'accepted' then
...
create trigger trg_guard_assistant_verification_approval before update on public.assistant_verification_requests
for each row execute function public.guard_assistant_verification_approval();

alter table public.assistant_verification_requests drop constraint if exists assistant_verification_requests_status_check;
alter table public.assistant_verification_requests add constraint assistant_verification_requests_status_check
check (status in ('pending','documents_requested','under_review','approved','rejected'));

-- Review decisions are admin-only. Assistants can upload new document rows but cannot mutate review_status.
