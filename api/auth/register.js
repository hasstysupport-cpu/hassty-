...
export default async function handler(req, res) {
  ...
  try {
    ...
    return jsonOk(res, { userId, maskedEmail: maskEmail(email), expiresIn: expiresInSeconds, resendAfter: CODE_RESEND_COOLDOWN, message: 'تم إنشاء حسابك بنجاح! أرسلنا رمز التفعيل إلى بريدك الإلكتروني — قد يصلك الرمز في صندوق الرسائل غير المرغوب فيها (Spam).', ttlMinutes: CODE_TTL_MINUTES });
  } catch (err) {
